...
class DbFuncoes
{
	...
	$strTipoRetorno = "1")
	{
		...
		//if($strTipoRetorno == "1")
		{
			//echo "strTipoRetorno=" . $strTipoRetorno; //debug.
		...
		//if($strTipoRetorno == "2-backup")
		{
			...
			if($idsTipoComplemento <> "")
			{
				...
				$idsTbCadastro = DbFuncoes::GetCampoGenerico07($nomeTabelaRelacaoComplemento, $nomeCampoReferencia, $nomeCampoComplemento, "");
				...
				if($idsTbCadastro <> "")
				{
					...
					$resultadoCadastroComplementoRelacao = DbFuncoes::FiltrosGenericosSelect02_FetchAll($nomeTabelaRelacaoComplemento);
					
					//echo "count(arrIdsTbCadastro)=" . count($arrIdsTbCadastro); //debug.
					//echo "count(arrIdsTipoComplemento)=" . count($arrIdsTipoComplemento); //debug.
					...
					for($countIdsTbCadastro = 0; $countIdsTbCadastro < count($arrIdsTbCadastro); $countIdsTbCadastro++)
					{
						...
						/**/for($countComplemento = 0; $countComplemento < count($arrIdsTipoComplemento); $countComplemento++)
						{
							if(empty($resultadoCadastroComplementoRelacao))
							{
								...
								foreach($resultadoCadastroComplementoRelacao as $linhaCadastroComplementoRelacao)
								{
									...
									if($linhaCadastroManutencao2[$nomeCampoComplemento] == $arrIdsTipoComplemento[$countComplemento] && $linhaCadastroManutencao2[$nomeCampoReferencia] == $arrIdsTbCadastro[$countIdsTbCadastro])
									{
						...
						{
							//echo "arrIdsTbCadastro[countIdsTbCadastro]=" . $arrIdsTbCadastro[$countIdsTbCadastro] . "<br />"; //debug.
		...
		//echo "strRetorno=" . $strRetorno . "<br />"; //debug.
		return $strRetorno;
	}
	//**************************************************************************************
	...
	//Função para retornar ids dos registros vinculados a um id de complementos usando uma sequencia de ids.
	//**************************************************************************************
	function GetIdsByTipoComplemento_FromArray03($idsTipoComplemento, 
	$nomeTabelaRelacaoComplemento, 
	$nomeCampoComplemento, 
	$nomeCampoReferencia, 
	$strTipoRetorno = "1")
	{
		...
		if($strTipoRetorno == "2")
		{
			...
			{
				$arrIdsTipoComplemento = explode(",", $idsTipoComplemento);
				$idsRegistros = DbFuncoes::GetCampoGenerico10($nomeTabelaRelacaoComplemento, 
																$nomeCampoReferencia, 
																array(), 
																"", 
																"", 
				...
				for($countComplemento = 0; $countComplemento < count($arrIdsTipoComplemento); $countComplemento++)
				{
					$idsRegistros = DbFuncoes::GetCampoGenerico10($nomeTabelaRelacaoComplemento, 
																$nomeCampoReferencia, 
																//array($nomeCampoComplemento . ";" . $arrIdsTipoComplemento[$countComplemento] . ";i"), 
																array($nomeCampoComplemento . ";" . $arrIdsTipoComplemento[$countComplemento] . ";i", $nomeCampoReferencia . ";" . $idsRegistros . ";ids"), 
																"", 
																"", 
																1);
					...
					//Debug.
					//echo "arrIdsTbCadastro[countIdsTbCadastro]=" . $arrIdsTipoComplemento[$countComplemento] . "<br />"; //debug.
					//echo "idsRegistros=" . $idsRegistros . "<br />"; //debug.
				...
				$idsRegistros = DbFuncoes::GetCampoGenerico10($nomeTabelaRelacaoComplemento, 
															$nomeCampoReferencia, 
															array($nomeCampoComplemento . ";" . $idsTipoComplemento . ";ids"), 
															"", 
															"", 
															1);
				*/
			...
			if($idsRegistros <> "")
			{
				//$strRetorno = $strRetorno . $idsRegistros . ",";
				$strRetorno = $idsRegistros;
			}
	...
	$tipoRetorno = 1)
	{
		...
		for($countArray = 0; $countArray < count($arrParametrosPesquisa); $countArray++)
		{
			...
			{
                $strSqlCampoGenericoSelect .= $strOperacaoPesquisa . " " . Funcoes::ConteudoMascaraGravacao01($parametrosPesquisaNomeCampo) . " = ? ";
			}
			if($parametrosPesquisaTipoCampo == "!s")
			{
                $strSqlCampoGenericoSelect .= $strOperacaoPesquisa . " " . Funcoes::ConteudoMascaraGravacao01($parametrosPesquisaNomeCampo) . " <> ? ";
			...
			if($parametrosPesquisaTipoCampo == "ids")
			{
                ...
                $strSqlCampoGenericoSelect .= $strOperacaoPesquisa . " " . Funcoes::ConteudoMascaraGravacao01($parametrosPesquisaNomeCampo) . " IN (" . $parametrosPesquisaValorCampo . ") ";
			}
			
			//Date.
			if($parametrosPesquisaTipoCampo == "d")
			{
                $strSqlCampoGenericoSelect .= $strOperacaoPesquisa . " " . "DATE(" . Funcoes::ConteudoMascaraGravacao01($parametrosPesquisaNomeCampo) . ") = ? ";
			}
			...
			if($parametrosPesquisaTipoCampo == "dif")
			{
				...
				*/
				...
				$strSqlCampoGenericoSelect .= $strOperacaoPesquisa . " " . Funcoes::ConteudoMascaraGravacao01($parametrosPesquisaNomeCampo) . " BETWEEN ? AND ? "; //funcionando
